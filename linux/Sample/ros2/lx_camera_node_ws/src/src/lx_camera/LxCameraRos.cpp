...
LX_STATE Start() {
  ...
  if (is_depth || is_amp || is_xyz) {
    ...
    for (int i = 0; i < 9; i++)
      tof_info.k[i] = k[i], tof_info.r[i] = ex_intr[i],
      tof_info.d.push_back(d[i]);
  ...
  if (is_rgb) {
    ...
    double k[9]{intr[0], 0, intr[2], 0, intr[1], intr[3], 0, 0, 1};
    for (int i = 0; i < 9; i++)
      rgb_info.k[i] = k[i], rgb_info.d.push_back(d[i]);
...
LxCamera::LxCamera(DcLib *_lib) : Node("lx_camera_node") {
  ...
  pub_rgb_info = this->create_publisher<sensor_msgs::msg::CameraInfo>(
      "LxCamera_RgbInfo", 1);
  ...
  pub_tof_info = this->create_publisher<sensor_msgs::msg::CameraInfo>(
      "LxCamera_TofInfo", 1);
  pub_error =
  ...
  pub_temper = this->create_publisher<lx_camera_ros::msg::FrameRate>(
      "LxCamera_FrameRate", 1);
  pub_obstacle = this->create_publisher<lx_camera_ros::msg::Obstacle>(
      "LxCamera_Obstacle", 1);
  pub_cloud = this->create_publisher<sensor_msgs::msg::PointCloud2>(
      "LxCamera_Cloud", 1);
  pub_tf = this->create_publisher<geometry_msgs::msg::TransformStamped>(
      "LxCamera_TF", 1);

  auto cmd =
      this->create_service<lx_camera_ros::srv::LxCmd>("LxCamera_LxCmd", &LxCmd);
  auto lxi =
      this->create_service<lx_camera_ros::srv::LxInt>("LxCamera_LxInt", &LxInt);
  auto lxb = this->create_service<lx_camera_ros::srv::LxBool>("LxCamera_LxBool",
                                                              &LxBool);
  auto lxf = this->create_service<lx_camera_ros::srv::LxFloat>(
      "LxCamera_LxFloat", &LxFloat);
  auto lxs = this->create_service<lx_camera_ros::srv::LxString>(
      "LxCamera_LxString", &LxString);
  ...
  std::thread ros2_work(&LxCamera::run, this);
...
void LxCamera::run() {
  ...
  while (rclcpp::ok()) {
    ...
    auto sret = DcSetCmd(handle, LX_CMD_GET_NEW_FRAME);
    ...
    if (is_xyz) {
      ...
      if (DcGetPtrValue(handle, LX_PTR_XYZ_DATA, (void **)&xyz_data) ==
        ...
        pcl2.header.stamp.sec = nanoseconds / 1e9;
        pcl2.header.stamp.nanosec = nanoseconds % static_cast<int64_t>(1e9);
        pcl2.header.frame_id = "mrdvs";
        pub_cloud->publish(pcl2);
      } else
        RCLCPP_WARN(this->get_logger(), "%s",
                    string("Cloud point data is empty!").c_str());
    }
    ...
    if (is_depth) {
      ...
      if (dep_data) {
        ...
        RCLCPP_WARN(this->get_logger(), "%s",
                    string("Depth image is empty!").c_str());
    ...
    if (is_amp) {
      ...
      if (amp_data) {
        ...
        RCLCPP_WARN(this->get_logger(), "%s",
                    string("Amplitude image is empty!").c_str());
    ...
    if (is_rgb) {
      ...
      if (rgb_data) {
        ...
        RCLCPP_WARN(this->get_logger(), "%s",
                    string("RGB image is empty!").c_str());
      Check("LX_FLOAT_2D_IMAGE_FPS",
            DcGetFloatValue(handle, LX_FLOAT_2D_IMAGE_FPS, &f_val));
      rgb = f_val.cur_value;
    }
    if (is_amp || is_depth || is_xyz)
      tof_info.header.stamp = now, pub_tof_info->publish(tof_info);
    if (is_rgb)
      rgb_info.header.stamp = now, pub_rgb_info->publish(rgb_info);
    if (is_xyz)
      tf.header.stamp = now, pub_tf->publish(tf);
