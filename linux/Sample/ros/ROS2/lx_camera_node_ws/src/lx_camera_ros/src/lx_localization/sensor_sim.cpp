...
SensorSim::SensorSim() : Node("sensor_sim_node") {
  ...
  this->get_parameter<std::string>("laserpose_topic_name",
                                   laserpose_topic_name_);
  ...
  scan_publisher_ = this->create_publisher<sensor_msgs::msg::LaserScan>(
      laser_topic_name_, 10);
  odom_publisher_ =
      this->create_publisher<nav_msgs::msg::Odometry>(odom_topic_name_, 10);
  pose_publisher_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(
      laserpose_topic_name_, 10);
