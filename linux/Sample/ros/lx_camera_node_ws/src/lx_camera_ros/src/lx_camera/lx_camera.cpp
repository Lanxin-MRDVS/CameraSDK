...
int value = -1;                                 \
nh_->param<int>(#cmd, value, -1);               \
if (value >= 0){                                \
ROS_INFO("%s:%d",#cmd,value);                   \
...
void ImuDataCallback(LxImuData* data_ptr, void* usr_data) {
    ...
    int64_t nanoseconds =
        static_cast<int64_t>(data_ptr->imu_data.sensor_timestamp * 1e3);
    msg->header.stamp.sec = nanoseconds / 1e9;
    msg->header.stamp.nsec = nanoseconds % static_cast<int64_t>(1e9);
...
LxCamera::LxCamera() {
    ...
    pub_cloud_ = nh_->advertise<sensor_msgs::PointCloud2>("LxCamera_Cloud", 10);
    pub_lidarCloud_ = nh_->advertise<sensor_msgs::PointCloud2>("LxCamera_LidarCloud", 10);
    kPubImu = nh_->advertise<sensor_msgs::Imu>("LxCamera_Imu", 20);
    ...
    int log_level_ = 1;
    nh_->param<int>("log_level", log_level_, 1);
    std::string log_path_ = "";
    nh_->param<std::string>("log_path", log_path_, "./");
    ROS_INFO("Log level:%d, file path: %s", log_level_, log_path_.c_str());
    DcSetInfoOutput(log_level_, true, log_path_.c_str());

    //gpu
    int enable_gpu = 0;
    nh_->param<int>("enable_gpu", enable_gpu, 1);
    DcSetGpuEnable(enable_gpu);

    int jped_decode = 0;
    nh_->param<int>("jped_decode", jped_decode, 0);
    DcSetJpegDecodeMethod(jped_decode);

    //find and open
    std::string ip_ = "";
    nh_->param<std::string>("ip", ip_, "");
    ROS_INFO("ip: %s", ip_.c_str());
    if (!SearchAndOpenDevice(ip_))
        return;

    //set
    nh_->param<int>("is_depth", is_depth_, 0);
    nh_->param<int>("is_xyz", is_xyz_, 0);
    ROS_INFO("publish depth: %d xyz: %d", is_depth_, is_xyz_);
    if (is_xyz_ || is_depth_) {
        Check("LX_BOOL_ENABLE_3D_DEPTH_STREAM",
            DcSetBoolValue(handle_, LX_BOOL_ENABLE_3D_DEPTH_STREAM, true));
    }
...
bool LxCamera::SearchAndOpenDevice(std::string ip_) {
    // find device
    int devnum = 0;
...
int LxCamera::Start() {
    ...
    tof_camera_info_.roi.x_offset = int_value.cur_value;
    DcGetIntValue(handle_, LX_INT_3D_IMAGE_OFFSET_Y, &int_value);
    tof_camera_info_.roi.y_offset = int_value.cur_value;
    DcGetIntValue(handle_, LX_INT_3D_BINNING_MODE, &int_value);
    tof_camera_info_.binning_x = int_value.cur_value * 2;
    tof_camera_info_.binning_y = tof_camera_info_.binning_x;
    ...
    rgb_camera_info_.roi.x_offset = int_value.cur_value;
    DcGetIntValue(handle_, LX_INT_2D_IMAGE_OFFSET_Y, &int_value);
    rgb_camera_info_.roi.y_offset = int_value.cur_value;
    DcGetIntValue(handle_, LX_INT_2D_BINNING_MODE, &int_value);
    rgb_camera_info_.binning_x = int_value.cur_value * 2;
    rgb_camera_info_.binning_y = rgb_camera_info_.binning_x;
    ...
    if (is_depth_ || is_amp_ || is_xyz_) {
        ...
        if (GetCameraCalibration(handle_, 1, tof_calib)) {
            ...
            case LX_DISTORTION_RADTAN_5:
                tof_camera_info_.distortion_model = "LX_DISTORTION_RADTAN_5";
                break;
            case LX_DISTORTION_RADTAN_14:
                tof_camera_info_.distortion_model = "LX_DISTORTION_RADTAN_14";
                break;
            case LX_DISTORTION_FISHEYE:
                tof_camera_info_.distortion_model = "LX_DISTORTION_FISHEYE";
                break;
            case LX_DISTORTION_SCARAMUZZA:
                tof_camera_info_.distortion_model = "LX_DISTORTION_SCARAMUZZA";
                break;
            default:
                tof_camera_info_.distortion_model = "LX_DISTORTION_UNDEFINE";
                break;
            }
            std::copy(tof_calib.K.begin(), tof_calib.K.end(), tof_camera_info_.K.begin());
            tof_camera_info_.D.assign(tof_calib.D.begin(), tof_calib.D.end());
        }
        else {
            tof_camera_info_.K.assign(0.0);
            tof_camera_info_.D.clear();
        }
        ...
        if (DcGetPtrValue(handle_, LX_PTR_3D_EXTRIC_PARAM,
            ...
            for (int i = 0; i < 9; i++) {
                tof_camera_info_.R[i] = ex_intr[i];
            }
            for (int i = 9; i < 12; i++)
            {
                tof_camera_info_.P[i] = ex_intr[i];
            }
        }
    ...
    if (is_rgb_) {
        ...
        if (GetCameraCalibration(handle_, 0, rgb_calib)) {
            ...
            case LX_DISTORTION_RADTAN_5:
                rgb_camera_info_.distortion_model = "LX_DISTORTION_RADTAN_5";
                break;
            case LX_DISTORTION_RADTAN_14:
                rgb_camera_info_.distortion_model = "LX_DISTORTION_RADTAN_14";
                break;
            case LX_DISTORTION_FISHEYE:
                rgb_camera_info_.distortion_model = "LX_DISTORTION_FISHEYE";
                break;
            case LX_DISTORTION_SCARAMUZZA:
                rgb_camera_info_.distortion_model = "LX_DISTORTION_SCARAMUZZA";
                break;
            default:
                rgb_camera_info_.distortion_model = "LX_DISTORTION_UNDEFINE";
                break;
            }
...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr LxCamera::BuildPointCloudXYZRGB(
...
pcl::PointCloud<pcl::PointXYZ>::Ptr LxCamera::BuildPointCloudXYZ(
...
pcl::PointCloud<PointXYZIT>::Ptr LxCamera::BuildPointCloudXYZIT(
    ...
    while (ros::ok()) {
        ...
        if (is_depth_ || is_xyz_) {
            ...
            if (dep_data) {
                ...
                msg_depth->header.stamp = ros::Time().fromSec(one_frame->depth_data.sensor_timestamp / 1000000.0);
                ...
                if (is_xyz_ == 2) {
                    ...
                    if (DcGetPtrValue(handle_, LX_PTR_XYZIRT_DATA, (void**)&xyzirt_data) == LX_SUCCESS) {
                        auto data = static_cast<LxPointCloudData*>(xyzirt_data);
                        auto cloud = BuildPointCloudXYZIT(data);
                        sensor_msgs::PointCloud2 msg_lidarCloud;
                        pcl::toROSMsg(*cloud, msg_lidarCloud);
                        msg_lidarCloud.header.stamp = ros::Time().fromSec(data->timebase / 1000000.0);
                ...
                if (is_xyz_ == 1) {
                    ...
                    if (DcGetPtrValue(handle_, LX_PTR_XYZ_DATA, (void**)&xyz_data) == LX_SUCCESS) {
                        ...
                        auto cloud = BuildPointCloudXYZRGB(xyz_data, rgb_data, buff_len);
...
void LxCamera::PubTf(const tf::Transform& transform,
    const std::string& frame_id,
    const std::string& child_frame_id) {
    static tf::TransformBroadcaster br;
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), frame_id,
        child_frame_id));
}