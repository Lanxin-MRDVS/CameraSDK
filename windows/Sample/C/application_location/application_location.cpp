...
int ReadAndSendOdom(DcHandle handle)
{
    ...
    size_t odom_mark_len = strlen(odom_mark);
...
int main(int argc, char** argv)
{
    ...
    //for test laser_data
    LxLaser laser_data;
    laser_data.timestamp = 1212121212;
    laser_data.time_increment = 10.f;
    laser_data.angle_min = 0.1f;
    laser_data.angle_max = 0.9f;
    laser_data.angle_increment = 0.05f;
    laser_data.range_min = 0.1f;
    laser_data.range_max = 0.8f;
    laser_data.range_size = 1501.f;// 360.f;
    laser_data.ranges = new float[(int)laser_data.range_size];
    for (int i = 0; i < laser_data.range_size; i++) 
        laser_data.ranges[i] = laser_data.angle_min++;

    auto ssret = DcSpecialControl(handle, "SetLaserData", &laser_data);
    if (ssret != LX_SUCCESS) {
        std::cout << " @@@@@@@@@@@@@SetLaserData failed, ssret:" << ssret << std::endl;
    } else 
        std::cout << " #########SetLaserData success, ssret:" << ssret << std::endl;
    if (laser_data.ranges != NULL) {
        delete[] laser_data.ranges;
        laser_data.ranges = NULL;
    }
